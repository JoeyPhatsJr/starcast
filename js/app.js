...
import { sunAltitude, moonAltitude, moonIllumination, visiblePlanets } from './astro.js';
...
const state = {
  prefs: { lat: null, lon: null, name: '', tz: 'America/New_York', bortle: 5, units: 'imperial' },
  hours: [],
  days: [],
  daily: [],
  selectedDay: 0,
...
function savePrefs() {
  try {
    const { lat, lon, name, tz, bortle, units } = state.prefs;
    localStorage.setItem(PREFS_KEY, JSON.stringify({ lat, lon, name, tz, bortle, units }));
...
const ROUTES = { '': 'conditions', '#/': 'conditions', '#/charts': 'charts', '#/settings': 'settings', '#/help': 'help' };

function applyRoute() {
  route = ROUTES[location.hash] || 'conditions';
  UI.showView(state, route);
  if (route === 'charts' && state.status === 'ready') UI.renderCharts(state);
}
