...
- **Phase 1 — "Sky" tab, touch-drag panorama** (fully buildable & verifiable headless): bright-star catalog as static JSON (mag ≤ 5, ~1,600 stars, ~40KB from public-domain Yale BSC/HYG) + constellation lines; canvas 2D gnomonic projection around a view direction; drag to look around; stars/planets/moon/sun plotted for the **scrubbed hour** (reuse `astro.js` — add an azimuth counterpart to `altitudeOf`). Works on desktop too.
- **Phase 2 — AR mode button on that view**: `DeviceOrientationEvent` drives the view direction (iOS needs `requestPermission()` in a user gesture; Android needs `deviceorientationabsolute`); rotation-matrix conversion + screen-orientation compensation + low-pass filtering; **compass gives magnetic north — embed a small magnetic-declination table (~15KB) because the error (~13° in NYC) breaks star identification**; optional `getUserMedia` camera passthrough toggle. Sensors cannot be tested headless — expect a build → owner field-tests on phone → fix cycle.
