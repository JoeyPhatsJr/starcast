...
**Small deferred polish** (review-flagged, none urgent): dedupe near-coincident star labels in the catalog generator (α Centauri is labeled twice: "Rigil Kentaurus" + "Toliman"); scale wheel-zoom step by `deltaY` magnitude; clamp `wmm.declination` latitude away from ±90° (NaN at the exact pole); widen the star-cull margin under roll (corner pop-in during AR); dedupe the alt-clamp literals in `onOrientation` vs skymap's `ALT_MIN/MAX`.
